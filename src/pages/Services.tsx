        ...
        <div className='flex w-full items-center justify-evenly pt-10 flex-wrap gap-y-10'>
            {isLoading ? 'loading...' :
                data.map((service: Service) => (
                    <Link to={service.id.toString()} key={service.id} className='relative flex items-center gap-x-9 p-5 w-[30dvw] aspect-square box-border bg-white hover:shadow-secondary-color hover:shadow-lg'>
                        <img src={karkasi} alt='karkasis foto' className='w-full aspect-square object-cover' />
                        <h3 className='text-3xl text-main-color capitalize absolute bg-white'>{service.name}</h3>
                    </Link>
                ))
            }
